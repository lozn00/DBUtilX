...
import cn.qssq666.dbutilmodule.BuildConfig;
...
//        }
        ...
        public Method generateSetMethod(Class classs, Field field) {
            String getMethodName = cn.qssq666.db.ReflectUtils.getGetName(field);
            String setMethodName = cn.qssq666.db.ReflectUtils.getSetName(field);
        ...
        public static Field[] getJavaBeanAllFields(Class<?> srcClass, ILoopAcceptBreak iLoopAcceptBreak) {
            ...
            while (srcClass != null && srcClass != Object.class) {
                ...
                for (int i = 0; i < fieldsSrc.length; i++) {
                    ...
                    String getMethodName = cn.qssq666.db.ReflectUtils.getGetName(fieldSrc);
                    String setMethodName = cn.qssq666.db.ReflectUtils.getSetName(fieldSrc);
...
//                    Prt.d(TAG, "" + getMethodName + ":" + invoke + ",setMethod:" + setMethod);
     ...
     */
         ...
         * @param appContext
         */
        public static void installLeakCanary(Application appContext) {
            try {
...
//            com.squareup.leakcanary.LeakCanary.install(this);
        ...
        static cn.qssq666.db.ReflectUtils.OnFailListener mFailListener = new cn.qssq666.db.ReflectUtils.OnFailListener() {
    ...
    public static Object callMethod(Method method, Object methodAtObj, Object... objArr) {
        Object obj2 = null;
        if (method != null) {
            try {
                obj2 = method.invoke(methodAtObj, objArr);
            } catch (Throwable th) {
            }
        }
        return obj2;
    ...
    public static Object CallStaticMethod(Method method, Object... objArr) {
        ...
        return ReflectUtils.callMethod(method, null, objArr);
    }
...
//                field.setAccessible(true);
    ...
    public static Class getSuperRootClass(Class aClass) {
        Class genericSuperclass = aClass.getSuperclass();

        String superClassName = genericSuperclass.getName() + "";
//        Log.w(TAG, "SuperclassName:" + superClassName + ",Objectname:" + name + ",value:" + (genericSuperclass == null || superClassName.equals(name)) + ",current:" + aClass.getName());
        if (genericSuperclass == null || superClassName.equals(Object.class.getName())) {
            //实际上返回 null
            return aClass;
        } else {
            return getSuperRootClass(aClass.getSuperclass());
        }
    }
     ...
     * @return
     */
    public static Class classArrayTypeToClass(String fieldType) {
        if (fieldType.contains(";") && fieldType.startsWith("[L")) {//把自定义对象数组转换为array.
            String classArray = fieldType.substring(2, fieldType.length() - 1);
            try {
                Class<?> aClassArr = Class.forName(classArray);
                return aClassArr;
    ...
    public static boolean interfaceInstanceOf(Class aClass, Class aClassInterface) {
        Class[] interfaces = aClass.getInterfaces();
        if (interfaces == null) {
            return false;
        }
        for (Class currentInterfaceClass : interfaces) {
            if (currentInterfaceClass == aClassInterface) {
                return true;
            }
        }
...
//        Log.w(TAG, "SuperclassName:" + superClassName + ",Objectname:" + name + ",value:" + (genericSuperclass == null || superClassName.equals(name)) + ",current:" + aClass.getName());
                    if (types == null || superClassName.equals(Object.class.getName())) {
                        //实际上返回 null
                        return false;
                    } else {
                        return interfaceInstanceOf(superClass, aClassInterface);
                    }
                }
            }
        }
    ...
    public static Method findMethodByArgClassStr(Class<?> cls, String methodName, String... paramTypeNames) {
        for (Method method : cls.getDeclaredMethods()) {
            if (method.getName().equals(methodName)) {
                Class[] parameterTypes = method.getParameterTypes();
                if (paramTypeNames.length == parameterTypes.length) {
                    int i;
                    boolean find = true;
                    currentFor:
                    for (i = 0; i < paramTypeNames.length; i++) {
    ...
    public static Method findMethod(Class<?> cls, String methodName, Class<?> returnTypeClass, Class<?>... clsArr) {
        for (Method method : cls.getDeclaredMethods()) {
            if (method.getName().equals(methodName) && method.getReturnType() == returnTypeClass) {
                Class[] parameterTypes = method.getParameterTypes();
    ...
    public static Method findMethodByReturnTypeStr(Class aClass, String methodName, String returnTypeName, Class<?>... clsArr) {
        for (Method method : aClass.getDeclaredMethods()) {
            if (BuildConfig.DEBUG) {
                Log.w(TAG, "find method:" + method.getName() + ",returnType:" + method.getReturnType().getName() + ",arg length:" + method.getParameterTypes().length);
            }
            if (method.getName().equals(methodName) && method.getReturnType().getName().equals(returnTypeName)) {
                Class[] parameterTypes = method.getParameterTypes();
                boolean find = true;
                if (clsArr.length == parameterTypes.length) {
                    int i;
